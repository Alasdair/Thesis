...
lemma preimp_exchange:
  ...
  by (metis (no_types, hide_lams) UnCI empty_subsetI insert_subset l_prod_distl sup.absorb1)
...
proof (rule seq.star_inductl_one[rule_format])
  have "x \<cdot> \<Union>powers x \<subseteq> \<Union>powers x"
    ...
    using l_prod_def apply fastforce
    apply (simp add: powers_def)
    apply (erule exE)
...
proof
  ...
  proof -
    have f1: "\<And>x\<^sub>1. (lmap unl (lfilter is_left x\<^sub>1)::'a llist) \<triangleright> traj x\<^sub>1 \<triangleleft> (\<rr> x\<^sub>1::'b llist) = x\<^sub>1"
      by (simp add: lefts_def_var reinterleave)
    have f2: "\<And>x\<^sub>2 x\<^sub>1 x\<^sub>3. lmap (\<lambda>Q. Inl ()) (x\<^sub>2::('a + 'b) llist) = lfilter is_left (traj (x\<^sub>1::('a + 'b) llist)) \<or> lmap unl (lfilter is_left x\<^sub>1) \<noteq> lmap x\<^sub>3 x\<^sub>2"
      by (metis (no_types) lefts_def_var traj_lfilter_lefts)
    have "\<And>x\<^sub>1. lmap unl (lmap Inl x\<^sub>1::('a + 'b) llist) \<triangleright> traj (lmap Inl x\<^sub>1::(_ + 'b) llist) \<triangleleft> (\<rr> (lmap Inl x\<^sub>1)::'b llist) = lmap Inl x\<^sub>1" using f1
      by (metis all_lefts)
    hence f3: "lmap unl (lfilter is_left t) \<triangleright> traj (lmap Inl t'::('a + 'b) llist) \<triangleleft> (\<rr> (lmap Inl t')::'b llist) = lmap Inl t'"
      by (metis `lmap unl (lfilter is_left t) = lmap unl (lmap Inl t')`)
    have "\<And>x\<^sub>1. lmap (\<lambda>Q. Inl ()) (lmap Inl x\<^sub>1::('a + 'b) llist) = lfilter is_left (traj (lmap Inl x\<^sub>1::(_ + 'b) llist))" using f2
      by (metis all_lefts)
    hence "\<And>x\<^sub>1. lfilter is_left (traj (lmap Inl t'::('a + 'b) llist)) = lfilter is_left (traj (x\<^sub>1::('a + 'b) llist)) \<or> lmap unl (lfilter is_left x\<^sub>1) \<noteq> lmap unl (lfilter is_left t)" using f2
      by (metis `lmap unl (lfilter is_left t) = lmap unl (lmap Inl t')`)
...
lemma [simp]: "x \<sha> y \<subseteq> FIN \<longleftrightarrow> lfinite x \<and> lfinite y"
  apply (auto simp add: tshuffle_words_def FIN_def subset_iff)
  sorry
...
lemma rg_lem8: "X \<subseteq> FIN \<Longrightarrow> z \<in> X \<cdot> {z'} \<cdot> Y \<Longrightarrow> env R z \<Longrightarrow> env R z'"
  ...
  prefer 2
  apply simp
  using rg_lem6 rg_lem7 apply blast
  by (metis rg_lem6)
...
proof -
  ...
  have "\<forall>v u. \<exists>uu. lprefix (u::'a llist) v \<longrightarrow> u \<frown> uu = v" by (metis (no_types) lprefix_conv_lappend)
